 ...
 *       }
    ...
    function Strategy(options, verify) {
        ...
        this.name = 'facebook';
        this._profileURL = options.profileURL || 'https://graph.facebook.com/v2.8/me';
        this._profileFields = options.profileFields || null;
        this._enableProof = options.enableProof;
        this._clientSecret = options.clientSecret;
        this._nullEmptyResponse = options.nullEmptyResponse;
    }
...
// Inherit from `OAuth2Strategy`.
    ...
    /**
     * Proccess the incoming object and requested fields to null any parameter that is not returned from Graph API.
     *
     * @param {comma separated string} incomingList
     * @param {object} _json param from Graph API response
     * @return a new object to be appended on profile holder object
     */
    Strategy.prototype.nullableFields = function (incomingList, responseList) {
        try {
            var incomingListArr = incomingList.trim().split(","),
                supportArr = responseList,
                isListedOnResponse;
        ...
        incomingListArr.forEach(function (property) {
            isListedOnResponse = false;
            for (var element in responseList) {
                if (responseList.hasOwnProperty(element)) {
                    if (property === element) {
                        isListedOnResponse = true;
                        break;
                    }
                }
            }

            if (!isListedOnResponse) {
                if (property) {
                    supportArr[property] = null;
                }
    ...
    Strategy.prototype.userProfile = function(accessToken, done) {
        ...
        this._oauth2.get(url, accessToken, function (err, body, res) {
            ...
            profile._raw = body;
            profile._json = json;
            if (self._nullEmptyResponse) {
                profile._processed = Strategy.prototype.nullableFields(fields, profile._json);
            }
